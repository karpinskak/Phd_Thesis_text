...
\begin{figure*}
\centering
\noindent \includegraphics[width=30pc]{gfx/Amax_vs_R_.png}
...
Figure \ref{fig:ch3_11} presents the same relation in a slightly different form. There are just a few $\delta$ values explored. Each coloured band represents $A_{max}$ range obtained for a bundle of alignment angles $\theta \in (0,\pi/2)$. While increasing $R$, for small particle radii, $A_{max}$ increases almost linearly with $R$, then the relation reaches maximum and $A_{max}$ decreases with $R$. The linear dependency for $\theta=0$ agrees with the relation shown in \ref{ch3:eq19a}, so $A_{max} \propto R/\delta$. The position of the maximum depends on $\theta$ and is practically independent of $\delta$ (at 17.10~$\mu$m for $\theta=\pi/2$ and $\delta=0.1$~cm, at 17.00~$\mu$m for $\theta=\pi/2$ and $\delta=0.9$~cm).
...
The dependence between $A_{max}$ and $R$ is clearly linear when $\theta=0$ in this range. In fact in such a case it agrees with the relation shown in \ref{ch3:eq19a}. However when increasing $\theta$, at some point the dependence between $A_{max}$ and $R$ becomes non-monotonic. It means that for an arbitrary $\delta$ and $\theta$ large enough, $A_{max}$ values are constrained from above. In other words, if a vortex has arbitrary $\delta$ and $\theta$ large enough, there is a value of $A$ above which there is no limit cycle solution for any particle size. The other way of looking at this solution is that for arbitrary $\delta$ and $\theta$ small enough so that the relation between $A$ and $R$ is monotonic, for a given $A$ there exist a range of particle sizes $R$ constrained only from below, that has limit cycle solutions. If $theta$ is large and the relation is non-monotonic, then this range is constrained from above as well. However the upper constrain is not considered here (??? najpierw sprawdzic, czy faktycznie gorne w tych zakresach jest mniejsze niz R2).



Finally we can formulate the general conclusion that increasing vortex core size $\delta$, or decreasing particle size when $\theta$ is small, causes the decrease in $A_{max}$ (equivalent to increase in minimal circulation $\Gamma_{min}$).\\
...\\
