...
\begin{figure*}
\centering
\noindent \includegraphics[width=30pc]{gfx/tau_z_error_vs_delta.png}
\caption{Characteristic time of the motion along vortex axis $\tau_z$ relative error versus vortex core size $\delta$ and particle radius $R$. Color indicates dependence on $R$. Plot variables' ranges correspond to cloud-like conditions.}
\label{fig:ch3_1}
\end{figure*}
...
\caption{Logarithmic factor in $\tau_{ex}$ vs. ratio of initial position and equillibrium position $a_2$. Line colors present cases of different ratios of vortex half-length and equillibrium position, $a_1$.}
\label{fig:ch3_2}
\end{figure*}
...
Figure \ref{fig:ch3_41} and Fig. \ref{fig:ch3_42} show particle docking in-orbit and on axis respectively, differing in parameters $St$ and $A$, for $\sigma=10^{-4}$. Line color corresponds to the value of parameter $St/A$. Fig.\ref{fig:ch3_43} and Fig.\ref{fig:ch3_44} represent the same data, but rescaled:
\begin{itemize}
\item $t^+$ is scaled for each particle separately by its $t^+_{doc}$.
\item radial velocity $\dot{r^+}$ is scaled by radial fluid velocity $u_r$ at the starting position
\item angular velocity $\varphi^+$ is scaled by rotational fluid velocity $u_{\varphi}^+(\sigma)/\sigma$ (axis docking) or circular orbit velocity $\omega_{orb}$ (in-orbit docking).
\end{itemize}

\begin{figure*}
\centering
\noindent \includegraphics[width=30pc]{gfx/orbit_docking_vel_traj_in_time_noscal.png}
\caption{Particle docking in orbit for different parameters $St$ and $A$, $\sigma=10^{-4}$. Line color corresponds to the value of parameter $St/A$. Panel \textbf{a} - radial coordinate,\textbf{b} - polar angle, \textbf{c} - radial velocity, \textbf{d} - angular velocity.}
\label{fig:ch3_41}
\end{figure*}
...
\caption{Same as in Fig. \ref{fig:ch3_41}, but time is scaled by each particles docking time, \textbf{c} - radial velocity is scaled by the fluid velocity  at the starting point $u_r(\sigma)$, \textbf{d} - angular velocity is scaled by the particle orbit angular velocity $\omega_{orb}$.}
\label{fig:ch3_43}
\end{figure*}
...
\caption{Particle docking on axis for different parameters $St$ and $A$, $\sigma=10^{-4}$. Line color corresponds to the value of parameter $St/A$. Panel \textbf{a} - radial coordinate,\textbf{b} - polar angle, \textbf{c} - radial velocity, \textbf{d} - angular velocity.}
\label{fig:ch3_42}
\end{figure*}
...
\caption{Same as in Fig. \ref{fig:ch3_42}, but time is scaled by each particles docking time, \textbf{c} - radial velocity is scaled by the fluid velocity  at the starting point $u_r(r_s)$, \textbf{d} - angular velocity is scaled by the fluid angular velocity $u_{\varphi}(\sigma)/\sigma$.}
\label{fig:ch3_44}
\end{figure*}

Figures \ref{fig:ch3_41} and \ref{fig:ch3_43} reveal that when the particle is docking in-orbit, it first followes the fluid motion around the axis, with radial velocity close to zero. With rotational velocity, the centrifugal force starts acting on it and causes a rapid increase in radial speed in the direction away from the vortex axis. Increasing distance from the axis in turn results in a steep decrease of the rotational velocity, and further the decrease of the radial velocity. Then the particle approaches its periodic orbit: radial velocity goes to zero, rotational velocity goes to $\omega_{orb}$. The rate of these changes depends on the parameters of the model in a complex way. After scaling the time by $t^+_{doc}$ it can be noticed that the docking process depends mainly monotonically on the parameter $St/A$. When its value decreases towards $16 \pi^2$ the docking time $t^+_{doc}$ tends to infinity.\\

\textit{Jakies pomysly, jak mozna opisac ten ruch kropli? Chodzi przeciez tylko o znalezienie skal czasowych ruchu, a nie o dokladne oszacowanie $t_{doc}$, ktory moze bardzo zalezec od warunkow poczatkowych i brzegowych.}\\

Now lets look closer at the axis docking process. Figures \ref{fig:ch3_42} and \ref{fig:ch3_44} show that  the particle firstly move with radial velocity the same as fluid's velocity: $v^+_r(r_s-\sigma)$. Next the particle radial velocity decreases almost exponentially in time. When the time is scaled by docking time, $t^+/t^+_{doc}$, the rate of exponential decrease depends on $St/A$ parameter only. Angular velocity increases towards fluid velocityclose to the axis, $v^+_{\varphi}(\sigma)/\sigma$, when particle is approching the axis . Another obsevation is that when $St/A$ increases towards the critical value $16 \pi^2$, the docking time $t^+_{doc}$  tends to infinity.\\
...
For $A<A_{cr}$ the equilibrium curve always has one maximum at $r^+_{max}$ and one minimum at $r^+_{min}$. The inflection point of $A=A _{cr}$ equilibrium curve plot lies at $r_i$ and for $S_{v i}$ (numerical estimations in the Table \ref{tab3_2}). It restricts values of $r^+_{max}$ from above and values of $r^+_{min}$ from below. Let's define:
\begin{align}
S_{v\ min}=f_A(r^+_{min}) \\
S_{v\ max}=f_A(r^+_{max})
\label{ch3:eq28b}
\end{align}
Consequently, for $S_v<S_{v\ min}$ and for $S_v>S_{v\ max}$, there is only one solution. For $S_v=S_{v\ min}$ and for $S_v=S_{v\ max}$, there are two solutions. For $S_{v\ min}<S_v<S_{v\ max}$, there are three solutions. All the conclusions are summarised in Table \ref{tab:ch3_1}.
...
Function $\phi(r^+_0)$ has only one zero at $r_s$ (see Table \ref{tab3_2}). For small radii, $r^+_0<r_s$, the equilibrium is a spiral/focus, and it is stable if:
...
In the case $A \geq A_{cr}$ the condition in Eq.\ref{ch3:eq31} is satisfied, because $A>\max_{r^+_0}\left( \sqrt{\varphi(r^+_0)}\right)=A_{cr}$. The unique solution is a stable node. These results are summarised in Table \ref{tab3_3}.
