...
At $r^{\ast}=0$ it has the same form as obtained for the case without gravity.\\
\noindent The (un)stability condition, which is to the best knowledge equivalent to existance of a limit cycle, after algebraic transformations comes down to the requirement that the strain parameter $A$ is small enough $A < A_{max}$ and consequently circulation $\Gamma$ large enough $\Gamma > \Gamma_{min}$, where the value is estimated as:
\begin{equation}
A_{max}=2^{-\frac{1}{2}} (4\pi)^{-1} \sqrt{(1+\nu^{-1}g^2\tau_p^3\sin^2\theta-\tau_p\gamma/2 \sqrt{1+\frac{2\tau_p\gamma}{1+\nu^{-1}g^2\tau_p^3\sin^2\theta-\tau_p\gamma/2}} - 1}.
\label{ch3:eq36}
\end{equation}
